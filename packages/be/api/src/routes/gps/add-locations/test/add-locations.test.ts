...
describe('POST /v1/gps/add-locations/:email/:sessionId (add location) ', () => {
  test('to return 400 on empty body', async () => {
    const response: request.Response = await request(app as App)
      .post('/v1/gps/add-locations/guiyep@gmail.com/222gh')
  ...
  test('to return 200 when correctly called', async () => {
    ...
    const response: request.Response = await request(app as App)
      ...
      .send({
        items: [
          { displayName: 'asd', lat: 2, long: 2 },
          { displayName: 'asd2', lat: 3, long: 4 },
        ],
      });
